...
public class ClienteDAO {
    ...
    public boolean update(Cliente cliente){
        ...
        for (int i = 0; i < BancoDAO.cliente.size(); i++) {
            if(idSaoIguais(BancoDAO.cliente.get(i),cliente)){
                BancoDAO.cliente.set(i, cliente);
                return true;
            }
        }
        return false;      
    ...
    public boolean delete(Cliente cliente){
        for (Cliente clienteLista : BancoDAO.cliente) {
            if(idSaoIguais(clienteLista,cliente)){
                BancoDAO.cliente.remove(clienteLista);
                return true;
            }
        }
        return false;
