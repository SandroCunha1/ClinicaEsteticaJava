...
public class ServicoDAO {
    ...
    public boolean update(Servico servico){
        ...
        for (int i = 0; i < BancoDAO.servico.size(); i++) {
            if(idSaoIguais(BancoDAO.servico.get(i),servico)){
                BancoDAO.servico.set(i, servico);
                return true;
            }
        }
        return false;      
    ...
    public boolean delete(Servico servico){
        for (Servico servicoLista : BancoDAO.servico) {
            if(idSaoIguais(servicoLista,servico)){
                BancoDAO.servico.remove(servicoLista);
                return true;
            }
        }
        return false;
